import { confirmApp } from "@shared/lib/appDialog";import { useState, useMemo, useRef, useEffect } from "react";
...
export default function HubMenu() {
  ...
  const handleAddonImageDelete = async (index) => {
    if (index < 0 || index >= addonImages.length) return;

    if (!(await confirmApp('Are you sure you want to delete this image?'))) {
      return;
    }
  ...
  const handleDeleteAddon = async (addon) => {
    if (!(await confirmApp(`Are you sure you want to delete "${addon.name}"? This action cannot be undone.`))) {
      return;
    }
  ...
  const handleDeleteCategory = async () => {
    ...
    if (!(await confirmApp(`Are you sure you want to delete the category "${selectedCategory.name}"? This will delete all items in this category.`))) {
      return;
    }
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-gray-500 ml-8">
                      This item will not be visible to customers on the taamio app till you switch it on.
                    </p>
                  </div>
                </div>
              </div>
