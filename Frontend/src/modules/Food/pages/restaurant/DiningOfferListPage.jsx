...
export default function DiningOfferListPage() {
  ...
  const loadOffers = async () => {
    try {
      ...
      setError(err?.response?.data?.message || "Failed to fetch dining offers");
    } finally {
      setLoading(false);
    }
  };
  ...
  const normalizedOffers = useMemo(
    ...
    offers.map((offer) => ({
      ...
      approvalStatus: String(offer?.approvalStatus || "pending").toLowerCase()
    })),
    [offers]
  );
  ...
  const handleDelete = async (id) => {
    ...
    if (!(await confirmApp("Delete this dining offer?"))) return;
    try {
      setDeletingId(id);
      await restaurantAPI.deleteDiningOffer(id);
      setOffers((prev) => prev.filter((offer) => String(offer?._id || offer?.id || "") !== id));
      setOpenMenuId("");
    } catch (err) {
      setError(err?.response?.data?.message || "Failed to delete dining offer");
    } finally {
      setDeletingId("");
    }
  };
                      ...
                      <AnimatePresence>
                        ...
                        <motion.div
                          ...
                          className="absolute right-0 top-9 z-50 min-w-[155px] rounded-xl border border-slate-200 bg-white p-1 shadow-xl">
                            ...
                            onClick={() => {
                              setOpenMenuId("");
                              navigate(`/food/restaurant/dining-offers/${offer.id}/edit`);
                            }}
