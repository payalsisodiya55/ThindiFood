        ...
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            ...
            <Input 
              ...
              className="pl-10 pr-10 h-11 bg-slate-100 dark:bg-zinc-800 border-none rounded-xl"
            ...
            <button 
              ...
              className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full transition-all ${isListening ? 'scale-125 animate-pulse' : 'text-slate-400'}`}
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4">
              {categories.map((cat) => (
                ...
                >
                  <div 
                    className={`w-14 h-14 rounded-2xl mb-2 flex items-center justify-center overflow-hidden border-2 transition-all ${selectedCategoryId === cat._id ? 'shadow-lg' : 'border-transparent bg-white dark:bg-zinc-900'}`}
                    style={selectedCategoryId === cat._id ? { borderColor: RED, shadowColor: `${RED}33` } : {}}
                  >
                    {cat.image ? (
                      <img 
                  ...
                  <span 
                    className={`text-[11px] font-medium text-center line-clamp-1 ${selectedCategoryId === cat._id ? '' : 'text-slate-600 dark:text-slate-400'}`}
                    style={selectedCategoryId === cat._id ? { color: RED } : {}}
                  >
                    {cat.name}
                  </span>
                </button>
                  ...
                  {results.dishes.map((r) => (
                    <Link to={`/user/restaurants/${r.slug || r._id}${r.matchedDishId ? `?dish=${r.matchedDishId}` : ''}`} key={r._id} className="flex gap-4 p-3 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-slate-100 dark:border-zinc-800 hover:shadow-md transition-shadow group">
                       ...
                       <div className="flex-1 min-w-0 flex flex-col justify-center">
                          <div className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: RED }}>
                          ...
                          <div className="flex items-center gap-3 text-xs text-slate-500 dark:text-zinc-400 mt-1">
                             <div className="flex items-center gap-1">
                                <Star className="w-3 h-3 fill-current" style={{ color: RED }} />
                  ...
                  {results.restaurants.map((r) => (
                    <Link to={`/user/restaurants/${r._id}`} key={r._id} className="block group">
                      ...
                      <div className="flex items-center justify-between px-1">
                         ...
                         <div 
                           className="text-[10px] font-bold bg-rose-50 dark:bg-rose-500/10 px-2 py-0.5 rounded-full uppercase tracking-wider"
                           style={{ color: RED }}
                         >
                            Top Pick
                         </div>
