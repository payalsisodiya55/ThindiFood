...
function RestaurantDetailsContent() {
  ...
  // Detect if restaurant has both veg and non-veg items
  const hasBothVegAndNonVeg = useMemo(() => {
    if (!restaurant?.menuSections || !Array.isArray(restaurant.menuSections)) return false;
    let hasVeg = false;
    let hasNonVeg = false;
    for (const section of restaurant.menuSections) {
      ...
      for (const item of allItems) {
        ...
        if (type === "non-veg" || type === "nonveg") hasNonVeg = true;
  ...
  return (
    <AnimatedPage
      id="scrollingelement"
      className={`min-h-screen bg-white dark:bg-[#0a0a0a] flex flex-col transition-all duration-300 ${shouldShowGrayscale ? 'grayscale' : ''
        }`}
    >
              ...
              <div className="flex items-center gap-2 flex-1 max-w-md">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2 rounded-full border border-gray-200 dark:border-gray-800 shadow-sm bg-white dark:bg-[#1a1a1a] text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-[#00c87e] focus:border-transparent"
                    autoFocus
                    onBlur={() => {
                      if (!searchQuery) {
                        setShowSearch(false)
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 py-4 sm:py-5 md:py-6 lg:py-8 space-y-3 md:space-y-4 lg:space-y-5 pb-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm overflow-hidden">
              <Tag className="h-4 w-4 text-[#00c87e]" />
              <div className="relative h-5 overflow-hidden">
                <AnimatePresence mode="wait">
                  <motion.span
                    ...
                    className="text-[#00c87e] font-medium inline-block"
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 whitespace-nowrap rounded-full border px-3 py-1.5 text-sm font-semibold transition-colors ${
                      selectedMenuCategory === "all"
                        ? "border-[#00c87e] bg-[#FEF2F2] text-[#00c87e]"
          ...
          <AnimatePresence>
            {showMenuSheet && (
              <>
                {/* Backdrop */}
                <motion.div
                  className={`fixed inset-0 bg-black/40 z-[9999] ${shouldShowGrayscale ? 'grayscale' : ''}`}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
          ...
          <AnimatePresence>
            {showFilterSheet && (
              <>
                ...
                {/* Bottom Sheet */}
                <motion.div
                  className={`fixed left-0 right-0 bottom-0 md:left-1/2 md:right-auto md:-translate-x-1/2 md:bottom-auto md:top-1/2 md:-translate-y-1/2 z-[10000] bg-white dark:bg-[#1a1a1a] rounded-t-3xl md:rounded-3xl shadow-2xl overflow-hidden h-auto max-h-[85vh] md:max-h-[90vh] w-full md:w-full md:max-w-2xl flex flex-col ${shouldShowGrayscale ? 'grayscale' : ''}`}
                  initial={{ y: "100%" }}
                  animate={{ y: 0 }}
                  exit={{ y: "100%" }}
                  transition={{ duration: 0.2, type: "spring", damping: 30, stiffness: 400 }}
                ...
                >
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 transition-all w-full ${filters.highlyReordered
                          ? "border-[#00c87e] dark:border-[#00c87e] bg-[#FEF2F2] dark:bg-[#00c87e]/20 text-[#00c87e] dark:text-[#00c87e]"
                          : "border-gray-200 dark:border-gray-700 bg-white dark:bg-[#2a2a2a] text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600"
                          }`}
                      >
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-800 px-4 py-3 flex items-center justify-between bg-white dark:bg-[#1a1a1a]">
                    ...
                    <Button
                      className="bg-[#00c87e] hover:bg-[#00c87e] text-white px-6 py-2.5 rounded-lg font-bold"
                      onClick={() => setShowFilterSheet(false)}
                    >
                    ...
                    {restaurant?.outlets && Array.isArray(restaurant.outlets) && restaurant.outlets.length > 0 ? (
                      <div className="space-y-2">
                        {restaurant.outlets.map((outlet) => (
                          ...
                          >
                            {outlet?.isNearest && (
                              <div className="flex items-center gap-1.5 mb-2 px-2 py-1 bg-[#FEF2F2] dark:bg-[#00c87e]/20 rounded-md">
                                <Zap className="h-3.5 w-3.5 text-[#00c87e] dark:text-[#00c87e] fill-[#00c87e] dark:fill-[#00c87e]" />
                                <span className="text-xs font-semibold text-[#00c87e] dark:text-[#00c87e]">
        ...
        createPortal(
          <AnimatePresence>
            {showShareModal && sharePayload && (
              <>
                <motion.div
                  className={`fixed inset-0 bg-black/50 z-[10020] ${shouldShowGrayscale ? 'grayscale' : ''}`}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
