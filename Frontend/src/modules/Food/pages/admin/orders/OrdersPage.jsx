import { confirmApp } from "@shared/lib/appDialog";import { useState, useEffect, useRef, useCallback, useMemo } from "react";
...
export default function OrdersPage({ statusKey = "all" }) {
  ...
  const normalizedOrders = useMemo(() => {
    ...
    return safeOrders.filter(Boolean).map((order) => {
      ...
      if (!paymentType) {
        if (paymentMethod === "cash" || paymentMethod === "cod") {paymentType = String(order.fulfillmentType || "").toLowerCase() === "takeaway" ? "Pay At Restaurant" : "Cash on Delivery";} else
        if (paymentMethod === "wallet") paymentType = "Wallet";else
        if (paymentMethod) paymentType = "Online";else
        paymentType = "N/A";
      }
      ...
      if (!paymentStatus) {
        ...
        if (refundStatus === "pending") paymentStatus = "Refund Pending";else
        if (refundStatus === "failed") paymentStatus = "Failed";else
        if (s === "refunded" || refundStatus === "processed") paymentStatus = "Refunded";else
        if (s === "paid" || s === "authorized" || s === "captured" || s === "settled") paymentStatus = "Paid";else
        if (s === "failed") paymentStatus = "Failed";
        // COD-aware resolution
        else if (isCodPayment && (s === "cod_pending" || s === "pending")) {
          if (isOrderDelivered) paymentStatus = "Collected (COD)";else
          if (isOrderCancelled) paymentStatus = "Not Charged";else
          paymentStatus = "COD Pending";
        }
        // Non-COD fallback
        else if (isOrderCancelled) paymentStatus = "Not Charged";else
  ...
  const handleDeleteOrder = async (order) => {
    ...
    const shouldDelete = await confirmApp(
