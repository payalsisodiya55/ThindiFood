...
export default function RestaurantPrivacyPolicy() {
  ...
  const fetchPrivacyData = async () => {
    try {
      setLoading(true)
      const response = await api.get(API_ENDPOINTS.ADMIN.RESTAURANT_PRIVACY, { contextModule: "admin" })
      if (response.data.success) {
        const content = response.data.data.content || ""
        const textContent = legalHtmlToPlainText(content)
        setPrivacyData({
          ...response.data.data,
          title: response.data.data.title || "Restaurant Privacy Policy",
          content: textContent
        })
      }
    } catch (error) {
      debugError("Error fetching restaurant privacy data:", error)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(
        API_ENDPOINTS.ADMIN.RESTAURANT_PRIVACY,
      ...
      debugError("Error saving restaurant privacy policy:", error)
    ...
    <div className="h-full overflow-y-auto bg-slate-50 p-4 lg:p-6">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-600 mt-1">Manage restaurant Privacy Policy content</p>
        </div>
