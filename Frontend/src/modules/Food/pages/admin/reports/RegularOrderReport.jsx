...
export default function RegularOrderReport() {
      ...
      try {
        ...
        if (response.data?.success) {
          ...
          const transformedOrders = rawOrders.map((order) => {
            ...
            const discountBreakdown = getDiscountBreakdown(pricing, order)
            const couponDiscount = discountBreakdown.totalDiscount
            const computedTotal =
            ...
            return {
              ...
              restaurantCouponDiscount: discountBreakdown.restaurantCouponDiscount,
              restaurantOfferDiscount: discountBreakdown.restaurantOfferDiscount,
              vatTax,
              deliveryCharge,
              platformFee,
              totalAmount,
              ...
              discountBreakdown: [
                `Platform: ${discountBreakdown.platformCouponDiscount}`,
                `Restaurant coupon: ${discountBreakdown.restaurantCouponDiscount}`,
                `Restaurant offer: ${discountBreakdown.restaurantOfferDiscount}`,
              ].join(" | "),
  ...
  const handleExport = (format) => {
    ...
    const headers = [
      ...
      { key: "platformCouponDiscount", label: "Coupon by Admin" },
      { key: "restaurantCouponDiscount", label: "Coupon by Restaurant" },
      { key: "restaurantOfferDiscount", label: "Offer by Restaurant" },
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1300px]">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "40px" }}>SI</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "100px" }}>Order Id</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "130px" }}>Restaurant</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "130px" }}>Customer Name</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "100px" }}>Total Item Amount</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "90px" }}>Coupon Admin</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "90px" }}>Coupon Restaurant</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "90px" }}>Offer Restaurant</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "110px" }}>Restaurant Earning</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "110px" }}>Admin Commission</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "80px" }}>Vat/Tax</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "80px" }}>Platform Fee</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "100px" }}>Order Amount</th>
                  <th className="px-2 py-2 text-left text-[9px] font-bold text-slate-700 uppercase tracking-wider" style={{ width: "90px" }}>Status</th>
                </tr>
              </thead>
                ...
                {paginatedOrders.length === 0 ? (
                  ...
                  paginatedOrders.map((order, index) => (
                    <tr key={order.orderId} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-blue-600 hover:underline cursor-pointer">{order.orderId}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700 truncate block">{order.restaurant}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700 truncate block">{order.customerName}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700">{formatAmount(order.totalItemAmount)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700">{formatAmount(order.platformCouponDiscount)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700">{formatAmount(order.restaurantCouponDiscount)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700">{formatAmount(order.restaurantOfferDiscount)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] font-semibold text-emerald-600">{formatAmount(order.restaurantEarning)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] font-semibold text-blue-600">{formatAmount(order.adminCommission)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700">{formatAmount(order.vatTax)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] text-slate-700">{formatAmount(order.platformFee)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="text-[9px] font-bold text-slate-900">{formatAmount(order.totalAmount)}</span>
                      </td>
                      <td className="px-1 py-1">
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[8px] font-medium bg-slate-100 text-slate-700">
                          {order.orderStatus}
                        </span>
                      </td>
                    </tr>
