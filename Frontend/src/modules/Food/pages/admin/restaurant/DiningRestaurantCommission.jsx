...
export default function DiningRestaurantCommission() {
  ...
  const fetchBootstrap = async () => {
    try {
      setLoading(true)
      const response = await adminAPI.getDiningRestaurantCommissionBootstrap()
  ...
  const fetchCommissions = async () => {
    try {
      setLoading(true)
      const response = await adminAPI.getDiningRestaurantCommissions({})
  ...
  const handleToggleStatus = async (commission) => {
    try {
      await adminAPI.toggleDiningRestaurantCommissionStatus(commission._id)
      await fetchCommissions()
      toast.success('Dining commission status updated successfully')
    } catch (error) {
  ...
  const handleEdit = async (commission) => {
    try {
      setLoading(true)
      const response = await adminAPI.getDiningRestaurantCommissionById(commission._id)
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true)
      await adminAPI.deleteDiningRestaurantCommission(selectedCommission._id)
      await fetchCommissions()
      toast.success('Dining commission deleted successfully')
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedCommission) {
        await adminAPI.updateDiningRestaurantCommission(selectedCommission._id, payload)
        toast.success('Dining commission updated successfully')
      } else {
        await adminAPI.createDiningRestaurantCommission(payload)
        toast.success('Dining commission created successfully')
      }
      ...
      debugError('Error saving dining commission:', error)
      toast.error(error.response?.data?.message || 'Failed to save dining commission')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold text-slate-900">Dining Commission</h1>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-white p-0">
          <DialogHeader className="px-6 pt-6 pb-4 border-b border-slate-200">
            <DialogTitle className="text-lg font-semibold text-slate-900">
              {selectedCommission ? "Edit Dining Commission" : "Add Dining Commission"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 px-6 py-4">
            {/* Restaurant Info */}
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="max-w-md bg-white">
          <DialogHeader>
            <DialogTitle>Delete Dining Commission</DialogTitle>
          </DialogHeader>
          <div className="py-4">
